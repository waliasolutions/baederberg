...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return <div className="group overflow-hidden bg-white rounded-lg shadow-sm transition-all duration-300 hover:shadow-md" style={{
    ...
    transitionDelay: `${delay}ms`
      ...
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {images.length > 1 && <>
            ...
            <button onClick={nextImage} className="absolute right-3 top-1/2 transform -translate-y-1/2 z-20 w-8 h-8 flex items-center justify-center rounded-full bg-white/80 hover:bg-white transition-fast" aria-label="Nächstes Bild">
        ...
        <div className="absolute bottom-3 left-3 z-20 flex items-center gap-1">
          <div className="flex items-center gap-1 px-2 py-1 bg-white/90 backdrop-blur-sm rounded-full text-xs font-medium">
            ...
            <span className="delete all mentions of locations">{location}</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-1">
          {tags.map((tag, i) => <span key={i} className="inline-block px-2 py-0.5 text-xs rounded-full bg-secondary/50 text-primary/80">
              {tag}
            </span>)}
        </div>
      </div>
